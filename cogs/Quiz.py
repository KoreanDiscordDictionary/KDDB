...
class Quiz(commands.Cog):
    ...
    async def quiz_que(self, ctx):
        i = 1
        embed = discord.Embed(title = '퀴즈 목록', description = ' ', color = 0x7289DA, inline=False)
        for i in self.coll.find({}):
            idd = i['_id']
            title = i['title']
            des = i['des']
            num = i['num']
            embed.add_field(name = f'{title}', value = f'퀴즈 아이디 : {idd}\n\n퀴즈 설명 : {des}\n선택지 : {num}')
        await ctx.send(embed=embed)
    ...
    async def solve_quiz(self, ctx, *, quizid: str):
        if self.coll.find_one({"_id": str(quizid)}):
            ...
            embed = discord.Embed(title = f'{title}', description = f'퀴즈 아이디 : {idd}\n\n퀴즈 설명 : {des}\n선택지 : {n}', color = 0x7289DA, inline=False)
            await ctx.send(embed=embed)
